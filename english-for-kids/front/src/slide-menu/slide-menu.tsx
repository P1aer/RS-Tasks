...
function SlideMenu(props:PropsFromRedux):React.ReactElement {
  ...
  return (
      <ul className={classes.join(" ")}>
          <NavLink onClick={() => { props.exitMenu(); props.stopGame(); }}
              className="menu-item" to="/" exact={true}>
              Main Page
          </NavLink>
          <NavLink to="/statistic" className={"menu-item"} onClick={() => { props.exitMenu(); props.stopGame(); }}>
              Statistic
          </NavLink>
          {
              cards.map((card) => (
                  <NavLink onClick={() => { props.exitMenu(); props.stopGame(); }}
                      className={"menu-item"} to={`/${card.name}`} key={card.id}>{card.name}
                  </NavLink>))
          }
